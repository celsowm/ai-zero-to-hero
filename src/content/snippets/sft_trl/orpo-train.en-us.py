...
def make_orpo_config(**kwargs) -> ORPOConfig:
    """
    Creates ORPOConfig in a way compatible with different TRL versions.

    Some experimental ORPOConfig versions do not accept the same fields
    as DPOConfig, for example truncation_mode. This helper filters out
    arguments the installed version does not know about.
    """
    ...
    if ignored:
        print("Arguments ignored by this ORPOConfig version:")
        for key in ignored:
            print(" -", key)
...
def make_orpo_trainer(
    *,
    ...
    """
    Creates ORPOTrainer in a way compatible with different TRL versions.

    Some versions use processing_class=tokenizer.
    Others use tokenizer=tokenizer.
    """
    ...
    if "processing_class" in accepted:
        ...
        print("Warning: ORPOTrainer exposes neither processing_class nor tokenizer in __init__.")
...
def main() -> None:
    ...
    print("Loading preference dataset:", DATASET_ID)
    ...
    args = make_orpo_config(
        ...
        # ORPO starts from the base model, not from an SFT checkpoint.
        # If the result is too weak, try 2e-6.
        # If it memorises or becomes too rigid, try 5e-7.
        learning_rate=1e-6,
        num_train_epochs=3,
        max_steps=-1,
