...
def drop_extra_columns(dataset: Dataset) -> Dataset:
    ...
    if remove:
        print("Removendo colunas extras:", remove)
...
def load_orpo_dataset() -> tuple[Dataset, Dataset]:
    ...
    print("Colunas train antes:", train_dataset.column_names)
    print("Colunas eval  antes:", eval_dataset.column_names)
    ...
    print("Colunas train depois:", train_dataset.column_names)
    print("Colunas eval  depois:", eval_dataset.column_names)
...
def detect_lora_target_modules(model: torch.nn.Module) -> list[str]:
    ...
    if not targets:
        raise RuntimeError("Não consegui detectar módulos Linear para LoRA.")

    print("=" * 100)
    print("Target modules LoRA detectados:")
...
def merge_adapter(tokenizer: AutoTokenizer, dtype: torch.dtype) -> None:
    print("=" * 100)
    print("Fazendo merge do adapter LoRA no modelo base")
    print("Base:", MODEL_ID)
...
def main() -> None:
    ...
    print("Carregando tokenizer:", MODEL_ID)
    ...
    print("Adapter final:", ADAPTER_DIR)
    print("Merged final:", MERGED_DIR)
    print("=" * 100)
    ...
    print("=" * 100)
    print("Salvando adapter LoRA em:", ADAPTER_DIR)
