...
\`\`\``,
      codeExplanations: [
        ...
        { lineRange: [53, 60], content: 'Retorna `(y, (k, v))` se `use_cache`. `_shape()`: reshape `(B, T, C)` → `(B, H, T, D)`.' },
      ],
    },
    'en-us': {
...
\`\`\``,
      codeExplanations: [
        ...
        { lineRange: [53, 60], content: 'Returns `(y, (k, v))` if `use_cache`. `_shape()`: reshapes `(B, T, C)` → `(B, H, T, D)`.' },
      ],
    },
  },
});
