        ...
        codePanels: [
          {
            ...
            codeExplanations: [
              ...
              { lineRange: [29, 43], content: 'Carrega o dataset completo do Hub e detecta o split automaticamente: usa `validation`, depois `test`, ou cria um 90/10 com `train_test_split`.' },
              { lineRange: [45, 53], content: 'Carrega tokenizer com `trust_remote_code`, configura `pad_token = eos_token` e `padding_side = "right"`.' },
              { lineRange: [55, 66], content: 'Carrega modelo com BF16 (GPU) ou FP32 (CPU). Desabilita cache, ativa gradient checkpointing e `enable_input_require_grads` para compatibilidade com LoRA.' },
              ...
              { lineRange: [151, 156], content: 'Calcula e exibe contagem de parâmetros treináveis e totais antes do treino.' },
              { lineRange: [158, 169], content: 'Treina o modelo, exibe métricas finais, salva adapter LoRA e tokenizer no diretório de saída.' },
              { lineRange: [172, 173], content: 'Guarda `main()` para execução como script.' },
            ],
          },
        ],
      },
        ...
        codePanels: [
          {
            ...
            codeExplanations: [
              ...
              { lineRange: [29, 43], content: 'Loads the full Hub dataset and auto-detects splits: uses `validation`, then `test`, or creates a 90/10 split with `train_test_split`.' },
              { lineRange: [45, 53], content: 'Loads tokenizer with `trust_remote_code`, sets `pad_token = eos_token` and `padding_side = "right"`.' },
              { lineRange: [55, 66], content: 'Loads model with BF16 (GPU) or FP32 (CPU). Disables cache, enables gradient checkpointing and `enable_input_require_grads` for LoRA compatibility.' },
              ...
              { lineRange: [151, 156], content: 'Calculates and prints trainable and total parameter counts before training.' },
              { lineRange: [158, 169], content: 'Trains the model, prints final metrics, saves LoRA adapter and tokenizer to the output directory.' },
              { lineRange: [172, 173], content: 'Standard `main()` guard for script execution.' },
            ],
          },
        ],
      },
