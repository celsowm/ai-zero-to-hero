  ...
  visual: {
    ...
    copy: {
      "pt-br": {
        ...
        "phaseExplanations": {
          "init": "Nesta fase a rede ainda não fez conta nenhuma sobre a amostra atual. O trecho iluminado só declara os pesos `w1`, `w2` e os vieses `b1`, `b2`. Em outras palavras: estamos vendo a configuração inicial da arquitetura, não o resultado do forward.\n\nO ponto didático aqui é separar claramente estrutura e execução. Primeiro aparecem os parâmetros que cada neurônio vai usar; só depois aparecem `z1`, `h1`, `z_out`, `y_hat` e os gradientes. Isso evita a impressão errada de que os números do painel nasceram automaticamente junto com a declaração dos pesos.",
          "forward": "Aqui o código foi escrito neurônio por neurônio de propósito. Em vez de esconder tudo em um `for`, o slide mostra `z1`, `z2` e `z3` de forma explícita para deixar visível que cada neurônio oculto faz sua própria soma ponderada. Logo abaixo, `h1`, `h2` e `h3` são as ativações sigmoid dessas três somas.\n\nDepois disso, a saída repete a mesma ideia: `z_out` combina `h1`, `h2` e `h3` com `w2` e `b2`, e `y_hat` aplica a sigmoid final. A versão compacta continua no comentário como equivalência, mas a forma expandida conversa 1:1 com o SVG e com os cards.",
          "backprop": "Depois do forward, o primeiro número importante é `erro_out = y_hat - alvo`. Esse termo mede a diferença bruta entre a previsão e a resposta correta. Em seguida, `loss = erro_out ** 2` transforma esse erro em erro quadrático, e `delta_out` converte isso no gradiente local da saída.\n\nA partir de `delta_out`, o erro volta para os neurônios ocultos. Cada `delta_h[i]` depende de três coisas: o peso `w2[i]`, a derivada local da sigmoid daquele neurônio e a intensidade do erro final. Por isso os três deltas ocultos quase nunca coincidem: cada neurônio contribuiu de um jeito diferente para a mesma previsão.",
          "update": "Aqui o código sai da explicação simbólica e entra no ajuste numérico dos parâmetros. `w2`, `b2`, `w1` e `b1` são atualizados usando a mesma regra: valor atual menos taxa de aprendizado vezes gradiente. Como os cards agora mostram apenas as variáveis do forward, loss e backprop, a mudança de pesos fica representada principalmente pela tabela do SVG.\n\nDidaticamente isso ajuda porque cada área cumpre um papel único: o código mostra a conta, os cards mostram as variáveis da conta atual, e o SVG mostra onde os pesos vivem e como eles mudam.",
          "finalize": "Quando o loop termina, o foco deixa de ser a amostra atual e passa a ser o estado aprendido da rede. O bloco final só empacota `w1`, `b1`, `w2` e `b2` em `parâmetros_finais`, que é exatamente o snapshot reutilizado no slide seguinte.\n\nEsse fechamento também reforça a divisão entre duas fases: durante o treino a rede produz `z`, `h`, `erro_out`, `loss` e `delta_*`; depois do treino, o que importa é guardar os parâmetros resultantes para inferência."
        },
        "tooltips": {
