        ...
        codePanels: [
          ...
          },
          {
            ...
            codeExplanations: [
              ...
              { lineRange: [53, 70], content: 'load_train_eval_dataset() baixa o dataset e cria splits validation/test com fallback para train_test_split.' },
              { lineRange: [73, 92], content: 'main(): cria pasta, fixa seed, diagnostica GPU, determina flags de precisão, ativa TF32, carrega dataset.' },
              { lineRange: [94, 110], content: 'Carregamos tokenizer (trust_remote_code, pad_token=eos_token, padding à direita) e modelo base com dtype adequado, cache desativado.' },
              { lineRange: [112, 132], content: 'SFTConfig: output_dir, 3 épocas, batch 16, sem acumulação, lr=1.5e-5, warmup=10, cosine, AdamW 8-bit, max_length=1024, packing=False.' },
              { lineRange: [134, 142], content: 'bf16/fp16/tf32 conforme diagnóstico, gradient checkpointing com use_reentrant=False, use_cache=False.' },
              { lineRange: [144, 167], content: 'assistant_only_loss=True, loss_type="nll", eval/logging/save a cada 50 steps, keep 2, load_best, report_to=none, remove_unused_columns=False, sementes fixas.' },
              { lineRange: [170, 176], content: 'SFTTrainer sem PEFT — todos os 752M parâmetros do Qwen participam do gradiente.' },
              { lineRange: [178, 199], content: 'Auditamos parâmetros treináveis vs totais, exibimos tamanho dos splits, treinamos e salvamos o checkpoint completo com tokenizer.' },
              { lineRange: [201, 202], content: 'Entrypoint padrão: executa main() quando o script é chamado diretamente.' },
            ],
          },
        ],
      },
        ...
        codePanels: [
          ...
          },
          {
            ...
            codeExplanations: [
              ...
              { lineRange: [53, 70], content: 'load_train_eval_dataset() downloads the dataset and creates validation/test splits with a train_test_split fallback.' },
              { lineRange: [73, 92], content: 'main(): creates folder, sets seed, diagnoses GPU, determines precision flags, enables TF32, loads dataset.' },
              { lineRange: [94, 110], content: 'We load tokenizer (trust_remote_code, pad_token=eos_token, right padding) and base model with appropriate dtype, cache disabled.' },
              { lineRange: [112, 132], content: 'SFTConfig: output_dir, 3 epochs, batch 16, no accumulation, lr=1.5e-5, warmup=10, cosine, 8-bit AdamW, max_length=1024, packing=False.' },
              { lineRange: [134, 142], content: 'bf16/fp16/tf32 per diagnostic, gradient checkpointing with use_reentrant=False, use_cache=False.' },
              { lineRange: [144, 167], content: 'assistant_only_loss=True, loss_type="nll", eval/logging/save every 50 steps, keep 2, load_best, report_to=none, remove_unused_columns=False, fixed seeds.' },
              { lineRange: [170, 176], content: 'SFTTrainer without PEFT — all 752M Qwen parameters participate in gradient updates.' },
              { lineRange: [178, 199], content: 'We audit trainable vs total parameters, display split sizes, train, and save the full checkpoint with tokenizer.' },
              { lineRange: [201, 202], content: 'Standard entrypoint: runs main() when the script is called directly.' },
            ],
          },
        ],
      },
