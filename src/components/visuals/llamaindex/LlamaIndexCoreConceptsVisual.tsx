...
import type { LlamaIndexCoreConceptsCopy } from '../../../types/slide';
import { sw } from '../../../theme/tokens';

interface LlamaIndexCoreConceptsProps {
  copy: LlamaIndexCoreConceptsCopy;
}

export const LlamaIndexCoreConceptsVisual = React.memo(({ copy }: LlamaIndexCoreConceptsProps) => {
  const [activeStep, setActiveStep] = useState(0);

  const steps = [
    { label: copy.conceptDoc, icon: '📄', color: sw.purple, detail: copy.documentLabel },
    { label: copy.conceptIndex, icon: '📇', color: sw.cyan, detail: copy.indexLabel },
    { label: copy.conceptRetriever, icon: '🔍', color: sw.pink, detail: copy.retrieverLabel },
    { label: copy.conceptQuery, icon: '❓', color: '#10b981', detail: copy.queryEngineLabel },
    { label: copy.conceptChat, icon: '💬', color: '#f59e0b', detail: copy.chatEngineLabel },
  ];
