...
type BioHoverTarget = 'dendrites' | 'soma' | 'integration' | 'firing';
type CompHoverTarget = 'inputs' | 'synapses' | 'weightedSum' | 'activation';

export const BiologicalVsComputationalNeuron = React.memo(({ copy }: Props) => {
  const [activeTab, setActiveTab] = useState(0);
  const [bioHover, setBioHover] = useState<BioHoverTarget | null>(null);
  const [compHover, setCompHover] = useState<CompHoverTarget | null>(null);
  ...
  const handleBioRowHover = useCallback((target: BioHoverTarget | null) => {
    setBioHover(target);
  }, []);

  const handleCompRowHover = useCallback((target: CompHoverTarget | null) => {
    setCompHover(target);
  }, []);
            ...
            <div className="flex flex-col gap-6 pt-2">
              ...
              <div className="w-full rounded-2xl bg-black/20 border border-white/5 overflow-hidden flex items-center justify-center p-4" style={{ aspectRatio: '16/7' }}>
                <svg viewBox="0 0 560 220" className="w-full h-full drop-shadow-2xl">
                  ...
                  {/* Dendrites group */}
                  <g
                    className={`transition-all duration-200 ${bioHover === 'dendrites' ? 'animate-svg-hover-pulse' : ''}`}
                    style={{
                      filter: bioHover === 'dendrites' ? 'url(#bio-glow)' : 'none',
                      opacity: bioHover && bioHover !== 'dendrites' ? 0.3 : 0.7,
                    }}
                  >
                    <g stroke="#4ade80" fill="none" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  {/* Soma group */}
                  <g
                    className={`transition-all duration-200 ${bioHover === 'soma' ? 'animate-svg-hover-pulse' : ''}`}
                    style={{
                      filter: bioHover === 'soma' ? 'url(#bio-glow)' : 'none',
                      opacity: bioHover && bioHover !== 'soma' ? 0.3 : 1,
                    }}
                  >
                    <path
                      d="M 135 78 C 162 70, 174 95, 176 112 C 172 134, 152 148, 132 143 C 105 138, 97 110, 106 87 C 114 74, 126 80, 135 78 Z"
                      fill={bioHover === 'soma' ? '#22c55e' : '#16a34a'}
                      stroke="#15803d"
                      strokeWidth="2"
                    />
                    <circle cx="138" cy="110" r="13" fill="var(--sw-cyan)" stroke="#0891b2" strokeWidth="2" />
                  </g>
                  {/* Axon + Myelin (integration) group */}
                  <g
                    className={`transition-all duration-200 ${bioHover === 'integration' ? 'animate-svg-hover-pulse' : ''}`}
                    style={{
                      filter: bioHover === 'integration' ? 'url(#bio-glow)' : 'none',
                      opacity: bioHover && bioHover !== 'integration' ? 0.3 : 1,
                    }}
                  >
                    ...
                    <g fill={bioHover === 'integration' ? '#4ade80' : '#22c55e'}>
                  ...
                  {/* Synapse (firing) group */}
                  <g
                    className={`transition-all duration-200 ${bioHover === 'firing' ? 'animate-svg-hover-pulse' : ''}`}
                    style={{
                      filter: bioHover === 'firing' ? 'url(#bio-glow)' : 'none',
                      opacity: bioHover && bioHover !== 'firing' ? 0.3 : 1,
                    }}
                  >
                    <circle cx="505" cy="178" r="36" fill="rgba(0,229,255,0.05)" stroke="var(--sw-cyan)" strokeWidth="1" strokeDasharray="4 4" />
                    <path d="M 476 152 C 484 162, 505 162, 509 176 C 508 189, 492 193, 479 184 Z" fill={bioHover === 'firing' ? '#22c55e' : '#16a34a'} stroke="#15803d" strokeWidth="1.5" />
                ...
                <table className="w-full text-xs text-left">
                  ...
                  <tbody className="text-[var(--sw-text-dim)]">
                    {copy.biologyTable.rows.map((row, i) => {
                      const targetMap: BioHoverTarget[] = ['dendrites', 'soma', 'integration', 'firing'];
                      const target = targetMap[i];
                      const isHighlighted = bioHover === target;
                      return (
                        <tr
                          key={i}
                          className="border-b border-white/5 last:border-0 transition-colors cursor-pointer"
                          style={{
                            backgroundColor: isHighlighted ? 'rgba(74,222,128,0.12)' : undefined,
                          }}
                          onMouseEnter={() => handleBioRowHover(target)}
                          onMouseLeave={() => handleBioRowHover(null)}
                        >
                          {row.map((cell, j) => (
            ...
            <div className="flex flex-col gap-6 pt-2">
              <div className="w-full rounded-2xl bg-black/20 border border-white/5 overflow-hidden flex items-center justify-center p-4" style={{ aspectRatio: '16/7' }}>
                <svg viewBox="20 0 520 220" className="w-full h-full drop-shadow-2xl">
                  ...
                  {/* Inputs group */}
                  <g
                    className={`transition-all duration-200 ${compHover === 'inputs' ? 'animate-svg-hover-pulse' : ''}`}
                    style={{
                      filter: compHover === 'inputs' ? 'url(#comp-glow)' : 'none',
                      opacity: compHover && compHover !== 'inputs' ? 0.3 : 1,
                    }}
                  >
                  ...
                  {/* Synapses (weights) group */}
                  <g
                    className={`transition-all duration-200 ${compHover === 'synapses' ? 'animate-svg-hover-pulse' : ''}`}
                    style={{
                      filter: compHover === 'synapses' ? 'url(#comp-glow)' : 'none',
                      opacity: compHover && compHover !== 'synapses' ? 0.3 : 1,
                    }}
                  >
                  ...
                  {/* Weighted sum + bias group */}
                  <g
                    className={`transition-all duration-200 ${compHover === 'weightedSum' ? 'animate-svg-hover-pulse' : ''}`}
                    style={{
                      filter: compHover === 'weightedSum' ? 'url(#comp-glow)' : 'none',
                      opacity: compHover && compHover !== 'weightedSum' ? 0.3 : 1,
                    }}
                  >
                    <circle cx="320" cy="140" r="44" fill={compHover === 'weightedSum' ? 'rgba(251,191,36,0.20)' : 'rgba(251,191,36,0.10)'} stroke="var(--sw-yellow)" strokeWidth="2.5" />
                  ...
                  {/* Activation group */}
                  <g
                    className={`transition-all duration-200 ${compHover === 'activation' ? 'animate-svg-hover-pulse' : ''}`}
                    style={{
                      filter: compHover === 'activation' ? 'url(#comp-glow)' : 'none',
                      opacity: compHover && compHover !== 'activation' ? 0.3 : 1,
                    }}
                  >
                    <circle cx="440" cy="140" r="26" fill={compHover === 'activation' ? 'rgba(168,85,247,0.20)' : 'rgba(168,85,247,0.10)'} stroke="var(--sw-purple)" strokeWidth="2.5" />
                ...
                <table className="w-full text-xs text-left">
                  ...
                  <tbody className="text-[var(--sw-text-dim)]">
                    {copy.computationTable.rows.map((row, i) => {
                      const targetMap: CompHoverTarget[] = ['inputs', 'synapses', 'weightedSum', 'activation'];
                      const target = targetMap[i];
                      const isHighlighted = compHover === target;
                      return (
                        <tr
                          key={i}
                          className="border-b border-white/5 last:border-0 transition-colors cursor-pointer"
                          style={{
                            backgroundColor: isHighlighted ? 'rgba(168,85,247,0.12)' : undefined,
                          }}
                          onMouseEnter={() => handleCompRowHover(target)}
                          onMouseLeave={() => handleCompRowHover(null)}
                        >
                          {row.map((cell, j) => (
                            <td key={j} className="px-4 py-3 font-medium">{cell}</td>
                          ))}
